...
const ExerciseVideos = ({ exerciseVideos ,name}) => {
  ...
  return (
    <Box sx={{marginTop:{lg:'200px',xs:'20px'}}} p="20px">
      ...
      Watch <span style={{color:'#ff2625',fontWeight:'bold',textTransform:'capitalize'}}>{name} </span> exercise videos
      </Typography>
      <Stack justifyContent={"flex-start"} flexWrap="wrap" alignItems="center" sx={{ flexDirection: { lg: 'row' }, gap: { lg: '110px', xs: '0' } }}>
     {exerciseVideos?.slice(0,6).map((item,index)=>(
       <a href={`https://www.youtube.com/watch?v=${item.video.videoId}`} key={index} className="exercise-video">
